// src/views/Compras/CuentasPorPagar.jsx
import React, { useEffect, useMemo, useState } from 'react'
import {
  CButton, CModal, CModalHeader, CModalBody, CModalFooter,
  CFormInput, CFormLabel, CForm, CCard, CCardBody, CCardHeader,
  CCol, CRow, CSpinner, CFormSelect,
} from '@coreui/react'
...
// ======== PALETA Y CLASES (solo UI, lógica intacta) ========
const COLORS = {
  title: '#2b2f3c',        // título oscuro (como Proveedores)
  cardBg: '#ffffff',       // card clara
  headerBg: '#f6f7fb',     // header clarito
}

const inputClasses = 'form-control'                         // inputs claros
const inputTextClasses = 'input-group-text bg-light text-muted'
...
const CuentasPorPagar = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setCuentas(arr.map(normalizarFila))
      } catch (err) {
  ...
  const handleGuardarPago = async () => {
    ...
    if (monto > Number(seleccion.saldo || 0)) return alert('El monto no puede ser mayor que el saldo pendiente.')

    try {
      setGuardandoPago(true)
      const data = await crearAbono(seleccion.id, { monto_pago: monto })

      setVisible(false)
      setMensajeExito(data?.message ?? `Se registró el pago por L. ${fmtLps(monto)} para ${seleccion.proveedor}.`)
  ...
  return (
    <CRow>
      <CCol>
        ...
        {/* Barra de búsqueda + acciones (alineadas como Proveedores) */}
        <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
          ...
          <div className="d-flex gap-2">
            ...
            <button className="btn btn-success" onClick={abrirNuevaCxP}>
              Agregar Cuenta Por Pagar
            </button>
          </div>
        </div>

        {/* Tabla en card clara como Proveedores */}
        <CCard className="mb-4 border-0 shadow-sm" style={{ borderRadius: 8, background: COLORS.cardBg }}>
          <CCardHeader className="d-flex align-items-center gap-2 border-0" style={{ background: COLORS.headerBg }}>
              ...
              <table className="table table-hover align-middle mb-0">
                ...
                <tbody>
                  ...
                  {cuentasFiltradas.map((cuenta) => (
                    <tr key={cuenta.id}>
                      <td className="fw-semibold">{cuenta.proveedor}</td>
                      <td>{cuenta.fechaCompra}</td>
                      <td>L. {fmtLps(cuenta.total)}</td>
                      <td>L. {fmtLps(cuenta.pagado)}</td>
                      <td className={Number(cuenta.saldo) > 0 ? 'text-danger' : 'text-success'}>
                        L. {fmtLps(cuenta.saldo)}
                      </td>
                      <td>
                        <span
                          className={`badge rounded-pill ${
                            cuenta.estado === 'Pagado' ? 'text-bg-success' : 'text-bg-warning text-dark'
                          }`}
                        >
                          {cuenta.estado}
                        </span>
                      ...
                      <td className="text-end">
                        <div className="d-inline-flex gap-2">
                          <button className="btn btn-outline-secondary btn-sm" onClick={() => abrirHistorial(cuenta)}>
                            Historial
                          </button>
                          <button
        ...
        {/* Modal de pago (mismos campos y lógica, solo estilo claro) */}
        ...
        {/* Modal Nueva CxP (mismos campos/lógica, estilo claro) */}
        <CModal visible={showNueva} onClose={() => setShowNueva(false)}>
          ...
          <CModalFooter>
            <CButton color="secondary" onClick={() => setShowNueva(false)}>Cancelar</CButton>
        ...
        {/* Modal Historial (lógica igual, tabla clara) */}
        <CModal visible={showHistorial} onClose={() => setShowHistorial(false)}>
          ...
          <CModalFooter>
            <CButton color="primary" onClick={() => setShowHistorial(false)}>Cerrar</CButton>
          </CModalFooter>
        </CModal>

        {/* Modal de éxito (idéntico) */}
        <CModal visible={mostrarExito} onClose={() => setMostrarExito(false)} centered>
          <CModalHeader>¡Operación exitosa!</CModalHeader>
          <CModalBody>
